...
void encabezadoLista() {
	...
	for (int np : p) {
		...
		case 8: {
			int x;
			cout << "\tObtiene el elemento en la posicion indicada (en este caso posicion 4) y lo almacena en la variable x\n\t";
			lista.print();
			lista.get(4, x);
			(x == 5 ? resultado = true : resultado = false);
			cout << "\n\tVariable x: " << x;
